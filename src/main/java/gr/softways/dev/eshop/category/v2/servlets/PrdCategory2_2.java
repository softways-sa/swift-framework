...
public class PrdCategory2_2 extends HttpServlet {
  
  private Director bean;

  private String _charset = null;
  
  private int _maxUploadSize = 3 * 1024 * 1024;
  ...
  public void init(ServletConfig config) throws ServletException {
    ...
    if (config.getInitParameter("maxUploadSize") != null) {
      _maxUploadSize = Integer.parseInt(config.getInitParameter("maxUploadSize"));
    }
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    ...
    try {
      ...
      multi = new MultipartForm(request, tmpPath, _maxUploadSize, _maxUploadSize, _charset);
    }
    catch (Exception e) {
      e.printStackTrace();
      dbRet.setNoError(0);
  ...
  private DbRet doInsert(HttpServletRequest request,String databaseId,MultipartForm multi,StrTokenizer denyFilesExtTokenizer,String uploadPath) {
    DbRet dbRet = new DbRet();
    ...
    String bgColor = multi.getParameter("bgColor");
    
    int imageMaxWidth = 0, imageMaxHeight = 0;
    
    try {
      imageMaxWidth = Integer.parseInt( multi.getParameter("imageMaxWidth") );
      imageMaxHeight = Integer.parseInt( multi.getParameter("imageMaxHeight") );
    }
    catch (Exception  e) {
    }
    ...
    String[] catImgName = new String[2];
    
    for (int i=0; i<2; i++) {
      String imgName = multi.getFileName("img" + (i+1));
      
      if (imgName == null) imgName = "";
      
      if (imgName.length() > 0) {
        imgName = SwissKnife.buildPK() + ".jpg";
        
        File file = multi.writeFile("img" + (i+1), uploadPath + "/" + imgName);
        
        processImage(file, imageMaxWidth, imageMaxHeight, bgColor);
      }
      
      catImgName[i] = imgName;
    }
    ...
    if (dbRet.getNoError() == 1) {
      try {
        ...
        ps.setString(30, catImgName[0]);
        ps.setString(31, catImgName[1]);
  ...
  private DbRet doUpdate(HttpServletRequest request,String databaseId,MultipartForm multi,StrTokenizer denyFilesExtTokenizer,String uploadPath) {
    DbRet dbRet = new DbRet();
    ...
    String[] oldimg = new String[] { SwissKnife.sqlEncode(multi.getParameter("oldimg1")), SwissKnife.sqlEncode(multi.getParameter("oldimg2")) };
    ...
    for (int i=0; i<2; i++) {
      ...
      if (imgName != null && imgName.length() > 0) {
        ...
        File file = multi.writeFile("img" + (i+1), uploadPath + "/" + imgName);
        if (file == null) dbRet.setNoError(0);
        
        if (dbRet.getNoError() == 1) processImage(file, imageMaxWidth, imageMaxHeight, bgColor);
        
        if (dbRet.getNoError() == 1 && oldimg[i].length() > 0) {
          File oldFile = new File(uploadPath + "/" + oldimg[i]);
          oldFile.delete();
        }
      }
      else imgName = oldimg[i];
      
      catImgName[i] = imgName;
    }
    
    if (dbRet.getNoError() == 0 || catId.length() == 0) {
      dbRet.setNoError(0);
    ...
    if (dbRet.getNoError() == 1) {
      try {
        ...
        ps.setString(29, catImgName[0]);
        ps.setString(30, catImgName[1]);
  ...
  private DbRet doDelete(HttpServletRequest request,String databaseId,MultipartForm multi,String uploadPath) {
    DbRet dbRet = new DbRet();
    ...
    String catId = SwissKnife.sqlEncode(multi.getParameter("catId")),
           catImgName1 = SwissKnife.sqlEncode(multi.getParameter("oldimg1")),
           catImgName2 = SwissKnife.sqlEncode(multi.getParameter("oldimg2"));
  ...
  private DbRet doDeleteImg(HttpServletRequest request,String databaseId,MultipartForm multi,String uploadPath) {
    DbRet dbRet = new DbRet();
    ...
    if (flag.equals("1")) {
      catImgName = SwissKnife.sqlEncode(multi.getParameter("oldimg1"));
    }
    else if (flag.equals("2")) {
      catImgName = SwissKnife.sqlEncode(multi.getParameter("oldimg2"));
    }
  ...
  private DbRet processImage(File item, int maxWidth, int maxHeight, String bgColor) {
    DbRet dbRet = new DbRet();
    ...
    try {
      ...
      if (image == null) throw new Exception("No suitable image reader found.");
      ...
      if (imageWidth < maxWidth && imageHeight < maxHeight) {
        ...
        x = (int)((maxWidth - thumbWidth) / 2);
        y = (int)((maxHeight - thumbHeight) / 2);
      }
      ...
      graphics2D.fillRect(0, 0, maxWidth, maxHeight);
