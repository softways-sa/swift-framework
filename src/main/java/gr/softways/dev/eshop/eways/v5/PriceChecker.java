...
public class PriceChecker {
  ...
  public static PrdPrice calcPrd(BigDecimal quantity,QueryDataSet dataSet,int customerType,boolean isOffer,BigDecimal discountPct) {
    ...
    return calcPrd(quantity,dataSet,customerType,isOffer,discountPct,giftWrap);
  }
  public static PrdPrice calcPrd(BigDecimal quantity,QueryDataSet dataSet,int customerType,boolean isOffer,BigDecimal discountPct, boolean giftWrap) {
    return calcPrd(quantity,dataSet,null,customerType,isOffer,discountPct,giftWrap);
  }
  
  public static PrdPrice calcPrd(BigDecimal quantity,QueryDataSet dataSet,ProductOptionsValue productOptionsValue,int customerType,boolean isOffer,BigDecimal discountPct) {
    boolean giftWrap = false;
    
    return calcPrd(quantity,dataSet,productOptionsValue,customerType,isOffer,discountPct,giftWrap);
  }
  public static PrdPrice calcPrd(BigDecimal quantity,QueryDataSet dataSet,ProductOptionsValue productOptionsValue,int customerType,boolean isOffer,BigDecimal discountPct,boolean giftWrap) {
    ...
    if (productOptionsValue == null) {
      ...
      if (Customer.CUSTOMER_TYPE_RETAIL == customerType && isOffer == true) {
        unitNetCurr1 = productOptionsValue.getPO_RetailOfferPrcEU();
      }
      else if (Customer.CUSTOMER_TYPE_RETAIL == customerType && isOffer == false) {
        unitNetCurr1 = productOptionsValue.getPO_RetailPrcEU();
      }
      else if (Customer.CUSTOMER_TYPE_WHOLESALE == customerType && isOffer == true) {
        unitNetCurr1 = productOptionsValue.getPO_WholesaleOfferPrcEU();
      }
      else if (Customer.CUSTOMER_TYPE_WHOLESALE == customerType && isOffer == false) {
        unitNetCurr1 = productOptionsValue.getPO_WholesalePrcEU();
      }
    }
    
    if (giftWrap == true) unitNetCurr1 = unitNetCurr1.add(dataSet.getBigDecimal(giftPrcEUColumn));
    
    return calcPrd(quantity,unitNetCurr1,dataSet.getBigDecimal("VAT_Pct"),Integer.parseInt(SwissKnife.jndiLookup("swconf/curr1Scale")),discountPct);
  }
  ...
  static private String giftPrcEUColumn = "giftPrcEU";
