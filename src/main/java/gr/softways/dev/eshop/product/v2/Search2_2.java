...
public class Search2_2 extends SearchBean2 {
  ...
  protected synchronized DbRet search(HttpServletRequest request) {
    ...
    from_clause = " FROM product JOIN prdInCatTab ON prdInCatTab.PINCPrdId = product.prdId JOIN prdCategory ON prdCategory.catId = prdInCatTab.PINCCatId JOIN VAT ON VAT.VAT_ID = product.PRD_VAT_ID "
                + "WHERE catShowFlag = '1'";
    ...
    if (inventoryType != null && inventoryType.equals(Order.STOCK_INVENTORY)) {
      from_clause += " AND product.stockQua > 0";
    }
