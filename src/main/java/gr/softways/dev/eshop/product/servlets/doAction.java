...
public class doAction extends HttpServlet {
  ...
  private DbRet doInsert(HttpServletRequest request, String databaseId, 
    ...
    for (int i=0; i<length; i++) {
      dbRet = insertImage(multi,imageUpload[i],denyFilesExtTokenizer,uploadPath);
  ...
  private DbRet doDelete(HttpServletRequest request, String databaseId, 
    ...
    String query = "DELETE FROM prdMonthly"
                 + " WHERE prdMId = '" + prdId + "'";

    dbRet = database.execQuery(query);

    if (dbRet.getNoError() == 1) {
      query = "DELETE FROM prdCompTab"
            + " WHERE pcomPrdCode = '" + prdId + "'";
      
      dbRet = database.execQuery(query);
    }
    
    if (dbRet.getNoError() == 1) {
      query = "DELETE FROM prdAttributes"
            + " WHERE prdAPrdId = '" + prdId + "'";

      dbRet = database.execQuery(query);
    }

    if (dbRet.getNoError() == 1) {
      query = "DELETE FROM PILines"
            + " WHERE PILPrdId = '" + prdId + "'";
      
      dbRet = database.execQuery(query);
    }
    
    if (dbRet.getNoError() == 1) {
      query = "DELETE FROM prdImports"
            + " WHERE prdId = '" + prdId + "'";
  ...
  private DbRet updateImage(String PLImage, MultiRequest multi,
  ...
  private DbRet insertImage(MultiRequest multi, String fname, 
