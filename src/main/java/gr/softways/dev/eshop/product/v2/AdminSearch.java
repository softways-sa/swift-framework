...
public class AdminSearch extends SearchBean2 {
  ...
  protected synchronized DbRet search(HttpServletRequest request) {
    ...
    if (_catId.length()>0) {
      select_clause += " JOIN prdInCatTab ON prdInCatTab.PINCPrdId = product.prdId JOIN prdCategory ON prdCategory.catId = prdInCatTab.PINCCatId"
          + " JOIN" 
    ...
    if (_catId.length()>0) {
      ...
      from_clause = " FROM product WHERE 1 = 1";
    }
