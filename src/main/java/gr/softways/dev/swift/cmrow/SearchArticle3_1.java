/**
 * nomizo pos einai mono sto paok..na to allakosume me tin SearchArticle3
 */
...
public class SearchArticle3_1 extends JSPBean {
  ...
  protected synchronized DbRet search(HttpServletRequest request) {
    ...
    from_clause = " FROM CMRow LEFT JOIN CMCRelCMR"
                + " ON CMRCode = CCCR_CMRCode WHERE CMCRelCMR.CCCRIsHidden <> '1'";
    ...
    if (_CMRCode.length() > 0) {
      from_clause += " AND CMRCode = '" + SwissKnife.sqlEncode(_CMRCode) + "'";
    }
